...
/** 계열사·별도 법인 조직 (S-NEXUS 대상 외) — 조직명으로만 제외 */
...
function collectAffiliateOrgIds(org: OrgSnapshot): {
  excludedDivisionIds: Set<string>;
  ...
  return { excludedDivisionIds, excludedTeamIds };
}

export function filterAffiliateOrg<T extends OrgSnapshot>(org: T): T {
  const { excludedDivisionIds, excludedTeamIds } = collectAffiliateOrgIds(org);
