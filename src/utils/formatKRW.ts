...
/** 매출액 순위 차트 상단 라벨 — 억원 정수 (소수점 없음) */
export const formatExecutiveKRWRankingLabel = (valInMillions: number | null | undefined): string => {
  if (valInMillions === null || valInMillions === undefined || Number.isNaN(valInMillions)) {
    return '0억원';
  }
  const eok = Math.round(valInMillions / 100);
