...
function divisionOrderStatus(ctx: AnalyticsChatContext, query: string): ChatbotResponse {
  ...
  return {
    text: `${summary}${divisionFilter ? ` (${divisionFilter})` : ''} 리스트를 정리했습니다.`,
    table,
...
export function buildMenuHelpResponse(): ChatbotResponse {
  return {
    text: `연동된 메뉴 데이터 전체에서 로컬 조회합니다. 예시:
· **프로젝트**: "올해 상반기 사업부별 수주 현황"
· **조직관리**: "조직 현황 보고서", "서석민이 누구야?"
· **구매관리·입찰**: "진행 중인 입찰 목록"
· **외주정보검색**: "석고보드 최근 6개월 단가"
· **인력배분**: "프로젝트 배분 현황"
· **전시사업비**: "전시사업 비용 유형별 현황"
· **대시보드**: "기여도 상위 현황"`,
  };
}
...
const PROJECT_QUERY_PATTERN =
  /프로젝트|수주|계약|공모|설계|제작|발주|contract|project|매출|billing|계약\s*변경|amendment|인사이트|보고서/i;

/** 프로젝트 관리 메뉴 로컬 집계 */
export function tryProjectLocalAnalyticsQuery(
  query: string,
  ctx: AnalysisIntegratedContext,
): ChatbotResponse | null {
  const normalized = query.trim();
  if (!normalized) return null;
  if (!PROJECT_QUERY_PATTERN.test(normalized) && !isInsightReportQuery(normalized)) return null;
  ...
  if (/프로젝트|수주|계약|공모|설계|제작/.test(normalized)) {
    return buildProjectInsightReport(ctx);
  }
