...
import { buildFacetedFilterOptionsForKey } from '@/utils/outsourcingAnalysis';
...
function OutsourcingMultiSelectFilterComponent({
  ...
  const [localSelected, setLocalSelected] = useState<string[]>(field.selected);
  ...
  useEffect(() => {
    ...
    setLocalSelected(field.selected);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      startTransition(() => {
        ...
        const nextOptions = buildFacetedFilterOptionsForKey(
  ...
  }, [open, allRecords, filters, filterKey, dateRange, facetedDependencyKey]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      startTransition(() => {
        ...
        onChange({ ...fieldRef.current, keyword: localKeyword, selected: localSelected });
  ...
  }, [field.keyword, localKeyword, localSelected, onChange]);
  ...
  useEffect(() => {
    ...
    startTransition(() => {
      ...
      onChange({ ...fieldRef.current, keyword: '' });

    });

  }, [activeFilterKey, field.keyword, filterKey, localKeyword, localSelected.length, onChange]);
  ...
  const commitSelected = (selected: string[]) => {
    ...
    startTransition(() => {
      ...
      onChange({ ...fieldRef.current, keyword: localKeyword, selected });
  ...
  const toggleValue = (value: string) => {
    ...
    if (selectedSet.has(value)) {
      ...
      commitSelected(localSelected.filter((item) => item !== value));

      return;

    }

    commitSelected([...localSelected, value]);
  ...
  const handleSelectAll = () => {
    ...
    const merged = new Set([...localSelected, ...filteredOptions]);
      ...
      <div className="outsourcing-filter-popover__toolbar">
        ...
        <span className="outsourcing-filter-popover__count">
          ...
          {optionsLoading
            ...
            : `${filteredOptions.length.toLocaleString('ko-KR')}건 · 선택 ${localSelected.length.toLocaleString('ko-KR')}건`}
      ...
      <div className="outsourcing-filter-field__header">
        ...
        {hasSelection && (
          ...
          >
            ...
            모두 삭제 ({localSelected.length.toLocaleString('ko-KR')})
...
function areMultiSelectFilterPropsEqual(
  ...
  return (
    ...
    prev.facetedDependencyKey === next.facetedDependencyKey &&

    prev.activeFilterKey === next.activeFilterKey &&

    prev.allRecords === next.allRecords &&
